...
export default function (fastify: FastifyInstance, opts: any, done) {
  ...
  fastify.options(
    ...
    async (request: PostRequest, reply: FastifyReply) => {
      try {
        ...
        console.error(e);
        const err = new Error(
          "Options request failed, see server logs for more details",
        );
  ...
  fastify.post(
    ...
    async (request: PostRequest, reply: FastifyReply) => {
      try {
        ...
        const viewer = adapter.createViewer(
          channelId,
        ...
        console.error(e);
        const err = new Error(
          "Exception thrown when handling a new WHPP client connection, see server logs for more details",
        );
        reply.code(500).send(err.message);
      }
    },
  ...
  fastify.options(
    ...
    {},
    async (request: PatchRequest, reply: FastifyReply) => {
      const channelId = request.params.channelId;
      ...
      try {
        ...
        console.error(e);
        const err = new Error(
          "Exception thrown see server logs for more details",
        );
        reply.code(500).send(err.message);
      }
    },
  ...
  fastify.put(
    "/channel/:channelId/:viewerId",
    {},
    async (request: PutRequest, reply: FastifyReply) => {
      try {
        const channelId = request.params.channelId;
        const viewerId = request.params.viewerId;
        ...
        if (!viewer) {
          console.error(
            `channelId ${channelId}, viewerId ${viewerId} not found`,
          );
          reply.code(404).send();
          return;
        }

        adapter.touchChannel(channelId);
        await viewer.handleAnswerRequest(request.body);
        reply.code(204).send();
      } catch (e) {
        console.error(e);
        const err = new Error(
          "Exception thrown when handling answer from WHPP client",
        );
        reply.code(500).send(err.message);
      }
    },
