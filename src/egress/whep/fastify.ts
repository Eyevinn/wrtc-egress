    ...
    async (request: PostRequest, reply: FastifyReply) => {
      try {
        if (request.body !== "") {
          reply
            .code(400)
            .send("SDP offer generated by the WHEP player not supported");
          return;
        }
        ...
        console.error(e);
        const err = new Error(
          "Exception thrown when handling a new WHEP client connection, see server logs for more details",
        );
        reply.code(500).send(err.message);
      }
    },
  ...
  fastify.patch(
    ...
    async (request: PatchRequest, reply: FastifyReply) => {
      try {
        ...
        console.error(e);
        const err = new Error(
          "Exception thrown when handling answer from WHEP client",
        );
        reply.code(500).send(err.message);
      }
    },
